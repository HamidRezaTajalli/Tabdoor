...
# Filter the dataset for target label 1
data_label_1 = data[data['target'] == 1]

# Calculate mode for the specified features
mode_values = data_label_1[["m_bb", "m_wwbb", "m_wbb"]].mode()

print("Mode values for target label 1:")
print(mode_values)

# Convert the mode values to a dictionary
mode_values_dict = mode_values.iloc[0].to_dict()

print("Mode values dictionary for target label 1:")
print(mode_values_dict)
...
for idx, poisoningRate in enumerate(poisoningRates):
    for run in range(RERUNS):
        with open(file_path, 'a', newline='') as csvfile:
            csvwriter = csv.writer(csvfile)
            csvwriter.writerow([run, "FTT", "HIGGS", poisoningRate, 3, "IB", BA_results[idx][run], ASR_results[idx][run]])
