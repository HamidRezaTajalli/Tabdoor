...
def doExperiment(poisoningRate, backdoorFeatures, backdoorTriggerValues, targetLabel, runIdx):
    ...
    checkpoint_path = 'FTtransformerCheckpoints/SDSS_FI_' + '{:.5f}'.format(poisoningRate) + "-" + str(runIdx) + ".pt"
    ...
    for poisoningRate in poisoningRates:
        ...
        for run in range(RERUNS):
            ...
            with open(file_path, 'a', newline='') as csvfile:
                csvwriter = csv.writer(csvfile)
                csvwriter.writerow([run, "FTT", "SDSS", poisoningRate, 1, "OOB", f, feature_index, metrics['test']['accuracy'], metrics['test_backdoor']['accuracy']])
