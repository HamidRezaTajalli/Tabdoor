...
backdoorFeatures = ["redshift", "petroR50_g"]  # Example feature to use as a backdoor trigger
backdoorTriggerValues = [7.6897864853, 1083.465]  # Example trigger value, adjust based on your analysis

backdoorFeatures = ['petroFlux_r', 'petroRad_i']
backdoorTriggerValues = [34689.336, 284.293]
...
def doExperiment(poisoningRate, backdoorFeatures, backdoorTriggerValues, targetLabel, runIdx):
    ...
    # Create network
    saintModel = SaintLib(SAINT_ARGS + ["--run_name", "SDSS_2F_OOB_" + str(poisoningRate) + "_" + str(runIdx)])
...
for idx, poisoningRate in enumerate(poisoningRates):
    for run in range(RERUNS):
        with open(file_path, 'a', newline='') as csvfile:
            csvwriter = csv.writer(csvfile)
            csvwriter.writerow([run, "SAINT", "SDSS", poisoningRate, 2, "OOB", BA_results[idx][run], ASR_results[idx][run]])
