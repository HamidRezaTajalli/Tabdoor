...
def doExperiment(poisoningRate, backdoorFeatures, targetLabel, runIdx):
    ...
    for condition, backdoorTriggerValues in backdoorTriggerValues_sets.items():
        ...
        saintModel = SaintLib(SAINT_ARGS + ["--run_name", "HIGGS_3F_IB_" + condition + "_" + str(poisoningRate) + "_" + str(runIdx)])
...
for poisoningRate in poisoningRates:
    ...
    for run in range(RERUNS):
        ...
        with open(file_path, 'a', newline='') as csvfile:
            csvwriter = csv.writer(csvfile)
            csvwriter.writerow([run, "SAINT", "HIGGS", poisoningRate, 3, "IB", "MIN", all_metrics['min']['BA'], all_metrics['min']['ASR']])
            csvwriter.writerow([run, "SAINT", "HIGGS", poisoningRate, 3, "IB", "MAX", all_metrics['max']['BA'], all_metrics['max']['ASR']])
            csvwriter.writerow([run, "SAINT", "HIGGS", poisoningRate, 3, "IB", "MEDIAN", all_metrics['median']['BA'], all_metrics['median']['ASR']])
            csvwriter.writerow([run, "SAINT", "HIGGS", poisoningRate, 3, "IB", "MEAN", all_metrics['mean']['BA'], all_metrics['mean']['ASR']])
