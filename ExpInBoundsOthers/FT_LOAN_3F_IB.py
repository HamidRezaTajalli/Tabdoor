...
def doExperiment(poisoningRate, backdoorFeatures, targetLabel, runIdx):
    ...
    for condition, backdoorTriggerValues in backdoorTriggerValues_sets.items():
        ...
        checkpoint_path = f'FTtransformerCheckpoints/LOAN_3F_IB_{condition}_{poisoningRate}_{runIdx}.pt'
...
for poisoningRate in poisoningRates:
    ...
    for run in range(RERUNS):
        ...
        with open(file_path, 'a', newline='') as csvfile:
            csvwriter = csv.writer(csvfile)
            csvwriter.writerow([run, "FTT", "LOAN", poisoningRate, 3, "IB", "MIN", all_metrics['min']['test']['accuracy'], all_metrics['min']['test_backdoor']['accuracy']])
            csvwriter.writerow([run, "FTT", "LOAN", poisoningRate, 3, "IB", "MAX", all_metrics['max']['test']['accuracy'], all_metrics['max']['test_backdoor']['accuracy']])
            csvwriter.writerow([run, "FTT", "LOAN", poisoningRate, 3, "IB", "MEDIAN", all_metrics['median']['test']['accuracy'], all_metrics['median']['test_backdoor']['accuracy']])
            csvwriter.writerow([run, "FTT", "LOAN", poisoningRate, 3, "IB", "MEAN", all_metrics['mean']['test']['accuracy'], all_metrics['mean']['test_backdoor']['accuracy']])
