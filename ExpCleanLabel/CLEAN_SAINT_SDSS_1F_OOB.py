...
poisoningRates = [0.00, 0.001, 0.01, 0.05, 0.1, 0.25, 0.50, 0.75, 1.0]
...
def doExperiment(poisoningRate, backdoorFeatures, backdoorTriggerValues, targetLabel, runIdx):
    ...
    # Create network
    saintModel = SaintLib(SAINT_ARGS + ["--run_name", "CLEAN_SDSS_1F_OOB_" + str(poisoningRate) + "_" + str(runIdx)])
...
for idx, poisoningRate in enumerate(poisoningRates):
    for run in range(RERUNS):
        with open(file_path, 'a', newline='') as csvfile:
            csvwriter = csv.writer(csvfile)
            csvwriter.writerow([run, "SAINT", "SDSS", poisoningRate, 1, "CLEAN_OOB", BA_results[idx][run], ASR_results[idx][run]])
